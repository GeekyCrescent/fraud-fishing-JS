...
export default function CrudUsuarios() {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/admin/user/stats", {
      ...
      .then((res) => res.json())
      .then((data) => {
        const arr = (data?.users ?? []) as UsuarioStats[];
        setUsuarios(arr);
        setPage(1);
      })
  ...
  // Ordenamiento por columna (por defecto, si eliges una)
  ...
  const handleCrear = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refetch para traer stats coherentes
  ...
  const handleEliminar = async (id: number) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/admin/user/${id}`, {
        method: "DELETE", // requiere DELETE en backend
        headers: { Authorization: "Bearer <TOKEN_ADMIN>" },
      });
      if (!res.ok) throw new Error();
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
          <h2 className="text-[22px] font-semibold tracking-tight">
            Usuarios: <span className="font-bold">{usuarios.length}</span>
          </h2>
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 w-72 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white"
              />
            </div>
            <button
            ...
            <button className="inline-flex items-center gap-2 border border-gray-200 hover:bg-gray-100 text-gray-700 px-3 py-2 rounded-lg">
              <FiSettings className="text-[18px]" />
              <span className="hidden sm:inline">Configurar tabla</span>
            </button>
          </div>
        </div>
          ...
          <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
              ...
              <div className="flex justify-end">
                <button className="bg-blue-600 text-white px-4 py-2 rounded" onClick={() => setDetalle(null)}>
                  Cerrar
                </button>
              </div>
            </div>
...
function RowUsuario({
  ...
  return (
    <tr className="border-t hover:bg-gray-50">
      ...
      <td className="py-4">{user.reportCount}</td>
      <td className="py-4">{user.commentCount}</td>
      <td className="py-4">{user.likeCount}</td>
      <td className="py-4">
